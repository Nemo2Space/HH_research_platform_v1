...
class AlphaDataLoader:
    ...
    def load_live_data(self, tickers: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        # =====================================================================
        # PATCH 3: Compute score_momentum from historical data
        # =====================================================================

        try:
            # Get historical scores for momentum calculation
            momentum_query = """
                ...
                SELECT DISTINCT ON (ticker)
                    ticker,
                    (total_score - total_score_ma20) AS score_momentum
                FROM recent_scores
                WHERE date >= CURRENT_DATE - INTERVAL '7 days'
                  AND total_score_ma20 IS NOT NULL
                ORDER BY ticker, date DESC
            """
            momentum_df = pd.read_sql(momentum_query, self.engine)

            if not momentum_df.empty:
                df = df.merge(momentum_df[['ticker', 'score_momentum']], on='ticker', how='left')
                logger.debug(f"Computed score_momentum for {momentum_df['ticker'].nunique()} tickers")
            else:
                df['score_momentum'] = np.nan
        except Exception as e:
            logger.warning(f"Could not compute score_momentum: {e}")
            df['score_momentum'] = np.nan
        ...
        # NOTE: score_momentum now computed from DB, not hardcoded
        # =====================================================================

        df['value_score'] = df['fundamental_score'] if 'fundamental_score' in df.columns else np.nan
        df['quality_score'] = df['fundamental_score'] if 'fundamental_score' in df.columns else np.nan
        ...
        df['gap_score'] = np.nan  # Let missingness handler deal with it
...
class MultiFactorAlphaModel:
    ...
    VERSION = "2.4.1"  # Soft regime adjustment, disabled rank normalization
    ...
    def train(self,
              ...
              n_folds: int = 5,
              purge_days: int = 5,
              use_rank_targets: bool = True) -> ModelValidationReport:
        """
        Train the model with walk-forward validation.

        FIX: Uses fold-wise fit/transform to avoid data leakage.
        PATCH 4: Train on cross-sectional ranks for better IC alignment.

        Args:
            use_rank_targets: If True, train on cross-sectional rank of returns
                             instead of raw returns. This aligns training with IC evaluation.
        """
        ...
        # =====================================================================
        # NOTE: Rank normalization was tested but made IC worse
        # Keeping raw feature values which performed better
        # =====================================================================

        self._use_rank_features = False  # Disabled - made IC worse
        self._use_rank_targets = use_rank_targets

        if use_rank_targets:
            logger.info("Using cross-sectional RANK targets for training")
            for horizon in self.target_horizons:
                ret_col = f'return_{horizon}d'
                rank_col = f'y_rank_{horizon}d'

                # Rank within each date, centered at 0
                df[rank_col] = df.groupby('date')[ret_col].transform(
                    lambda x: x.rank(pct=True) - 0.5 if len(x) > 1 else 0
                )

                # Store mapping for prediction scaling
                rank_std = df[rank_col].std()
                return_std = df[ret_col].std()
                self.return_stats[horizon]['rank_to_return_scale'] = return_std / rank_std if rank_std > 0 else 1.0
        else:
            logger.info("Using raw return targets for training")
    ...
    def _walk_forward_validation_fixed(self,
        ...
        for fold in range(n_folds):
            ...
            for horizon in self.target_horizons:
                # PATCH 4: Use rank targets if enabled
                if getattr(self, '_use_rank_targets', False):
                    y_col = f'y_rank_{horizon}d'
                    # Fallback to raw returns if rank column doesn't exist
                ...
                y_test_for_train = df_test[y_col].values if y_col in df_test.columns else df_test[f'return_{horizon}d'].values

                # For IC evaluation, always use raw returns (that's what we care about)
                y_test_actual = df_test[f'return_{horizon}d'].values

                # Remove NaN
                valid_train = ~np.isnan(y_train)
                valid_test = ~np.isnan(y_test_actual)

                if valid_train.sum() < 50 or valid_test.sum() < 10:
                    continue

                # Train model on this fold's training data (using rank or raw based on setting)
                ...
                y_actual = y_test_actual[valid_test]

                # Calculate IC against ACTUAL returns (not ranks)
                # This gives us the true predictive power
    ...
    def _train_final_models(self, df: pd.DataFrame, X: np.ndarray):
        ...
        PATCH 4: Uses rank targets if enabled for training,
        but evaluates IC against actual returns.
        """
        ...
        for horizon in self.target_horizons:
            ...
            y = df[y_col].values
            y_actual = df[f'return_{horizon}d'].values  # For IC calculation
            valid = ~np.isnan(y) & ~np.isnan(y_actual)
            ...
            # Calculate IC against ACTUAL returns
            ic = np.corrcoef(model.predict(X[valid]), y_actual[valid])[0, 1]
            ...
            self.global_weights[horizon] = FactorWeights(
                ...
                r_squared=r2_score(y_actual[valid], model.predict(X[valid])),
                mse=mean_squared_error(y_actual[valid], model.predict(X[valid])),
        ...
        if self.use_regime_models:
            for regime in df['regime'].unique():
                ...
                for horizon in self.target_horizons:
                    ...
                    y_actual = df.loc[regime_mask, f'return_{horizon}d'].values
    ...
    def predict(self,
        ...
        for horizon in self.target_horizons:
            ...
            # =====================================================================
            # PATCH 1: Ensemble with IC filtering
            # Only filter out NEGATIVE IC models (they hurt predictions)
            # Keep weak positive IC models in the ensemble
            # =====================================================================
            MIN_IC_THRESHOLD = 0.0  # Only filter negative IC (was 0.02, too strict)

            if preds:
                filtered_preds = []
                filtered_ics = []

                for pred_val, w in zip(preds, weights_used):
                    ic = getattr(w, "information_coefficient", None)
                    if ic is None:
                        # Include model but with low weight
                        filtered_preds.append(pred_val)
                        filtered_ics.append(0.01)
                        continue
                    try:
                        ic = float(ic)
                    except Exception:
                    ...
                    if ic < MIN_IC_THRESHOLD:
                        continue  # Only drop NEGATIVE IC models

                    # Use IC as weight, but ensure minimum weight
                    filtered_preds.append(pred_val)
                    filtered_ics.append(max(ic, 0.01))

                if filtered_preds:
                    # IC-weighted ensemble
                    total_ic = float(np.sum(filtered_ics))
                    ensemble_pred = float(np.sum([p * ic for p, ic in zip(filtered_preds, filtered_ics)]) / total_ic)
                    predictions[horizon] = ensemble_pred
                elif preds:
                    # Fallback: simple average if all models filtered
                    predictions[horizon] = float(np.mean(preds))
    ...
    def predict_live(self, tickers: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        PATCH 2: Apply cross-sectional rank normalization to match training.
        """
        if not self._is_trained:
            raise ValueError("Model not trained. Call train() first.")
        ...
        # =====================================================================
        # PATCH 2: Apply cross-sectional rank normalization to features
        # Must match what was done during training
        # =====================================================================

        if getattr(self, '_use_rank_features', False):
            feature_cols = [f for f in self.feature_names if not f.endswith('_missing')]
            for col in feature_cols:
                if col in df.columns:
                    # Rank across all stocks in this batch
                    df[col] = df[col].rank(pct=True)
            logger.debug(f"Applied rank normalization to {len(feature_cols)} features")
        ...
        # PHASE 3: Assign signals based on quantiles with SOFT REGIME ADJUSTMENT
        #
        # Based on validation report IC per regime:
        # - NEUTRAL: IC = +0.14 ✓ (good) → Full confidence
        # - HIGH_VOL: IC = +0.18 ✓ (excellent but N=71 small) → Full confidence
        # - BULL: IC = -0.06 ✗ (weak negative) → Reduced confidence
        # - BEAR: IC = -0.04 ✗ (weak negative) → Reduced confidence
        #
        # SOFT adjustment: reduce confidence in negative regimes but don't force HOLD
        # This allows some signals while being more conservative

        POSITIVE_IC_REGIMES = ['NEUTRAL', 'HIGH_VOL']
        NEGATIVE_IC_REGIMES = ['BULL', 'BEAR']
        ...
        for pred in raw_predictions:
            ...
            regime = pred.regime
            regime_favorable = regime in POSITIVE_IC_REGIMES

            # Quantile-based signal assignment (always generate, then adjust)
            ...
            # =====================================================================
            # SOFT REGIME ADJUSTMENT (not strict gate)
            # Reduce confidence but don't force HOLD
            # =====================================================================

            if regime in NEGATIVE_IC_REGIMES:
                # Model is weaker in this regime - reduce conviction
                conviction = max(0.35, conviction - 0.15)
                # Downgrade STRONG signals to regular
                if signal == "STRONG_BUY":
                    signal = "BUY"
                elif signal == "STRONG_SELL":
                    signal = "SELL"
            elif regime in POSITIVE_IC_REGIMES:
                # Model is reliable in this regime - slight boost
                conviction = min(0.95, conviction + 0.05)

            # Confidence based on strength and coverage
            distance = abs(prob_pos - 0.5)
            if distance > 0.06 and abs(pred_5d) > 0.3 and coverage > 0.7 and regime_favorable:
                confidence = "HIGH"
            ...
            # Downgrade if low coverage
            if coverage < 0.6:
                confidence = "LOW"
                conviction = max(0.4, conviction - 0.15)

            # Downgrade HIGH confidence in negative regimes
            if not regime_favorable and confidence == "HIGH":
                confidence = "MEDIUM"
    ...
    def save(self, path: str = 'models/multi_factor_alpha.pkl'):
        ...
        state = {
            ...
            # v2.4+ rank normalization flags
            'use_rank_features': getattr(self, '_use_rank_features', False),
            'use_rank_targets': getattr(self, '_use_rank_targets', False),
    ...
    def load(self, path: str = 'models/multi_factor_alpha.pkl'):
        ...
        # v2.4+: Restore rank normalization flags
        self._use_rank_features = state.get('use_rank_features', False)
        self._use_rank_targets = state.get('use_rank_targets', False)
...
def train_alpha_model(min_date: str = None,
                      ...
                      use_rank_targets: bool = False) -> ModelValidationReport:
    """Train and save a new alpha model.

    Args:
        use_rank_targets: If True, train on cross-sectional ranks.
                         Default False (raw returns performed better in testing).
    """
    model = MultiFactorAlphaModel()
    report = model.train(min_date=min_date, use_rank_targets=use_rank_targets)
    model.save(save_path)
    return report


def sanity_check_forward_returns(n: int = 200, horizon: int = 5) -> dict:
    """
    PATCH 2: Verify that stored return columns match actual forward returns.
    ...
    Returns:
        dict with 'correlation', 'sign_match_rate', 'n_samples', 'status'
    """
    ...
    # Get random sample of historical data
    query = f"""
        SELECT ticker, score_date as date, return_{horizon}d
    ...
    # Compare stored returns with computed returns from prices
    rows = []
    for r in df.itertuples(index=False):
        try:
            # Get prices at T and T+horizon
            price_query = """
                SELECT 
                    (SELECT close FROM prices WHERE ticker=%(t)s AND date=%(d)s LIMIT 1) AS p0,
                    (SELECT close FROM prices WHERE ticker=%(t)s AND date > %(d)s ORDER BY date LIMIT 1 OFFSET %(h)s) AS p1
            """
            d2 = (r.date + pd.Timedelta(days=horizon * 2)).date()  # Look ahead

            # Simpler approach: get closing prices
            p_query = f"""
                ...
                LIMIT {horizon + 5}
            """
            prices = pd.read_sql(p_query, engine)

            if len(prices) < horizon + 1:
                continue

            p0 = prices.iloc[0]['close']
            p1 = prices.iloc[min(horizon, len(prices)-1)]['close']

            if p0 is None or p1 is None or p0 == 0:
                continue

            realized = (float(p1) / float(p0) - 1.0) * 100.0
            stored = float(getattr(r, f'return_{horizon}d'))
    ...
    if len(rows) < 30:
        return {
            ...
            'message': f'Only {len(rows)} valid samples found',
            'n_samples': len(rows)
        }

    chk = pd.DataFrame(rows)
    correlation = chk['stored'].corr(chk['realized'])
    sign_match = (np.sign(chk['stored']) == np.sign(chk['realized'])).mean()

    # Determine status
    if correlation > 0.8 and sign_match > 0.7:
        status = 'OK'
        message = 'Forward returns appear correctly computed'
    elif correlation < 0:
        status = 'INVERTED'
        message = 'Returns may have inverted sign!'
    elif sign_match < 0.5:
        status = 'MISALIGNED'
        message = 'Returns may have date alignment issues'
    else:
        status = 'WARNING'
        message = 'Returns show moderate correlation - verify manually'
    ...
    result = {
        ...
        'n_samples': len(chk),
        'stored_mean': float(chk['stored'].mean()),
        'realized_mean': float(chk['realized'].mean()),
        'stored_std': float(chk['stored'].std()),
        'realized_std': float(chk['realized'].std())
    }

    print(f"Forward Return Sanity Check (horizon={horizon}d):")
    print(f"  Status: {status}")
    print(f"  Correlation: {correlation:.3f}")
    print(f"  Sign match: {sign_match:.1%}")
    print(f"  Samples: {len(chk)}")
    print(f"  Stored mean: {result['stored_mean']:.2f}%, Realized mean: {result['realized_mean']:.2f}%")
