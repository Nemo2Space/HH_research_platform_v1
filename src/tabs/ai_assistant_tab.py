...
def render_ai_assistant_tab():
    ...
    with col1:
        if st.button("🚀 Short-term Plays", use_container_width=True):
            _execute_research("What are the best stocks for short-term trading this week?")

    with col2:
        if st.button("📈 2025 Outlook", use_container_width=True):
            _execute_research("What stocks have the best outlook for 2025?")

    with col3:
        if st.button("💎 Long-term Holds", use_container_width=True):
    ...
    if DUAL_ANALYST_AVAILABLE:
        ...
        for i, ticker in enumerate(top_tickers):
            with [col1, col2, col3, col4][i]:
                if st.button(f"🔬 {ticker}", use_container_width=True, key=f"dual_{ticker}"):
...
def _render_dual_response(result: Dict[str, Any]):
    ...
    # Disclaimer
    st.caption("*This analysis combines quantitative metrics with qualitative SEC filing analysis. Not financial advice.*")
